...
Only use numeric claims when they come from an approved source, a user-provided current source, or an explicitly approved prior asset.
...
- product capabilities not yet live

When a claim is directionally useful but not verified for the specific account, use qualitative language.
...
Choose proof by relevance, not by largest number.
...
| Enterprise trust | major customer names, governance, security readiness |
...
Use current user approval or current public source checks before placing third-party logos into a new public artifact.
...
- predictive campaign planning unless the user supplies a current approved source
