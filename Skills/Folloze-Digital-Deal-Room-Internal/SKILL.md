...
- For Luke-owned opportunities, invite **Luke Rafferty** as a board **Editor** after the board exists and before declaring it ready for review. During preflight, resolve Luke's exact current organization user/invite ID and registered email or alias from the authoritative application response; do not assume `luke@folloze.com` is the current inviteable identity. Verify the stable identity plus Luke's display name in the board Editors readback; an invite request, search result, or success toast alone is not sufficient evidence.
...
- valid Experience API authentication, exact template `248623`, config/hash access, and preferred vanity availability
- authenticated designer/preview access and a known invitation-capable app/API path
- Luke Rafferty's exact current organization user/invite identity
