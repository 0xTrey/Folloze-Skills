...
2. Use the company theme only after the user has authorized the theme mode. For vendor-branded pages, use non-Folloze theme mode when already authorized by the user or by prior context. When the brief clearly says vendor-branded, Salesforce-branded, customer-branded, no Folloze branding, or Folloze-owned, ask the required theme-mode question during setup and record the answer in the research note instead of waiting until final save approval.
