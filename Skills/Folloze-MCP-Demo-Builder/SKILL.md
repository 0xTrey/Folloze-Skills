...
- For Trey's demo-pack-builder work, no Folloze company theme is permanently pre-authorized so the vendor's brand system stays in control. Call the theme tool with `use_folloze_theme: "no"`, record that inherited decision, and do not ask the theme question again. This standing authorization applies only when this skill is invoked by `folloze-demo-pack-builder`; preserve the normal approval boundary for unrelated one-off boards.
...
2. Use the company theme only after the user has authorized the theme mode. In demo-pack mode, Trey's standing authorization is always `no`: call `get_company_theme` with `use_folloze_theme: "no"`, record it, and do not ask again. Outside demo-pack mode, ask the required theme-mode question during setup and record the answer in the research note.
...
7. If the user is still in local-preview/review mode, stop before MCP save and return the local HTML path plus preview state. Demo-pack mode is the exception: once QA passes, save and publish automatically under the standing demo-pack delivery contract.
...
When invoked by `folloze-demo-pack-builder`, continue after save: publish the board, assign or update a unique descriptive vanity URL, verify the public URL anonymously, and update the canonical tracker row. A demo-pack board is not complete while any of those states is pending.
...
- Demo-pack mode is an explicit tracker-maintenance exception: create the row on first save, then update that same canonical row after publish or vanity-URL changes so it contains the final board ID, designer URL, verified public URL, QA status, and public-verification status. Never create a second row for the same board.
